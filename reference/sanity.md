...
| Trip          | `trip`           | name, slug, river (ref), activities (refs), categories (refs), difficulty, duration, description (Portable Text), highlights, minAge, season, featuredReview, itinerary, arcticTripId | Central content type. `arcticTripId` is the comma-separated Arctic trip-type id(s). |
