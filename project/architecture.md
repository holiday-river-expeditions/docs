...
- **Trip Finder** (`tripFinderSpec`, added 2026-09-04) — `questions[]` of `tripFinderQuestion` (kind, copy, image, weight, `onlyWhen`/`skipWhen` conditions, `options[]` of `tripFinderOption` with one kind-specific dial each), plus `minConfidentScore` and `resultsShown`. Read through `src/lib/trip-finder-spec.ts`, which Zod-validates it and falls back to `DEFAULT_TRIP_FINDER_SPEC` in code. Seeded from that default by `scripts/seed-trip-finder-spec.mjs`. See [[trip-finder]]
...
- **Current flags:** the logo treatments render as a radio group in the panel (mutually exclusive; CSS precedence legacy > fresh > secondary > line > bold > classic default): `logo-bold` (stacked live-text), `logo-line` (single line, medium weight), `logo-secondary` (the brand package's Secondary Horizontal, `public/logo-secondary-red.svg`), `logo-fresh` (HOLIDAY-dominant hierarchy exploration), `logo-legacy` (the pre-rebrand bikeraft.com lockup, `public/logo-legacy-red.svg` — guidelines forbid mixing old logos, demo only). Checkbox flags: `bars-on-scroll` (floating bars appear on scroll vs always), `sticky-header` (persistent header), `river-flow` (CFS chip + sparkline, hidden from real visitors until approved), `badge-live` (one-time stamp-in animation on the hero seal), `trip-finder` (the "Find Your Trip" wizard's entry points; the route itself is always reachable), `finder-logic` (the `TripFinderLogicPanel` explainer under the wizard and its results — full ranking with per-question breakdown, data sources, missing facts; added 2026-09-04). Client components read flags live via `useDemoFlag` (`src/lib/use-demo-flag.ts`). The stakeholder-facing guide is [[demo-panel-guide]].
