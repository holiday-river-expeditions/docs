...
**14 types as of 2026-08-28** — 12 documents + 2 object types. Activity and Trip Category were merged into a single **Trip Type** on 2026-08-28; **Trip Info Section** was added the same day.
...
- **Trip** — name, slug, river (ref), `tripType` (ref, required), duration, description (Portable Text), highlights, `whatsIncluded`, `videoUrl`, minAge, season, `featuredReview`, `itinerary` (array of itineraryDay objects), `infoSections`, and **`arcticTripId`** — the comma-separated Arctic trip-type id(s) linking a Sanity trip to live departures. Fields are grouped in the Studio (Basics, Card, Trip Details, Quick Facts, Booking, Trip Finder, Specialty); before 2026-08-28 all 44 sat in one flat column.
  ...
  - Trip-finder fields (added 2026-08-27 for the `/trip-finder` wizard scorer): `maxRapidClass` (I–V; now the only difficulty signal), `seasonMonths`, `minAgeOverrides`, `craftTypes`. **Seeded with placeholder values pending Holiday's confirmation** (min ages and seasons especially).
...
- **Current flags:** the logo treatments render as a radio group in the panel (mutually exclusive; CSS precedence legacy > fresh > secondary > line > bold > classic default): `logo-bold` (stacked live-text), `logo-line` (single line, medium weight), `logo-secondary` (the brand package's Secondary Horizontal, `public/logo-secondary-red.svg`), `logo-fresh` (HOLIDAY-dominant hierarchy exploration), `logo-legacy` (the pre-rebrand bikeraft.com lockup, `public/logo-legacy-red.svg` — guidelines forbid mixing old logos, demo only). Checkbox flags: `bars-on-scroll` (floating bars appear on scroll vs always), `sticky-header` (persistent header), `river-flow` (CFS chip + sparkline, hidden from real visitors until approved), `badge-live` (one-time stamp-in animation on the hero seal), `trip-finder` (the "Find Your Trip" wizard at `/trip-finder`, hidden until Holiday signs off). Client components read flags live via `useDemoFlag` (`src/lib/use-demo-flag.ts`).
